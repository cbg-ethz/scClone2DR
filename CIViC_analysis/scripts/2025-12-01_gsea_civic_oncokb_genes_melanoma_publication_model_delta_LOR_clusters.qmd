---
title: "GSEA of main model beta on civic gene sets"
...
output <- paste0(working_directory, "output/main/")
```
...
LOR_file <- paste0(input, "geneOncoKB_scatrex/_LOR.h5")
...
for (i_drug in seq_along(attrs$dim1_drugs)) {
  ...
  # Hydroxyurea = 19
  # i_drug = 19
  for (malclone in 1:8) {                       # Python's [1..8]
    #malclone = 1
...
From sample: "MACEGEJ" "MACYHYS" "MADEGOD" .. And cluster/clone: 0 1 2 3 .. Now combined: "MACEGEJ_0" "MACYHYS_1" "MADEGOD_1" ..
...
for (drug in seq_along(attrs$dim1_drugs)) {
  ...
  all_patients_list <- list()
    ...
    for (patient in seq_along(attrs$dim3_samples)) {
      # patient = 1
      #print(patient)
      patient_data <- as.data.frame(deltaLOR[drug, , patient, ])
  ...
  # "MAKYGIW_2" "MEKAKYD_1" "MEKAKYD_2" "MELUKUZ_1" "MELUKUZ_2" "MELUKUZ_7" "MIFOGIL_1" "MIFOGIL_2" "MIFOGIL_3" "MIFOGIL_4" "MYJILAS_1"
  #filter_mask_rows <- apply(md, 1, function(x) all(is.na(x)))
...
The drug Hydroxyurea contains missing values (NaN and/or -Inf) for all features in some clusters.

Those are removed and ignored so far.

For sample MIFOGIL not only one clusters is removed but all, because LOR of cluster 0 is NaN. Therefore, delta LOR is not possible.
...
for (my_drug in drugs_min_5_genes_in_overlap) {
  ...
  ggsave(filename = filename,plot = combined_plot, width = 30, height = 65, dpi = 300, limitsize = FALSE)
...
```{r}
# write results table
filename <- paste0(output, "main_GSEA_delta_LOR.tsv")
...
df_min5 <- df %>% 
  filter(size >= 5)
filename <- paste0(output, "main_GSEA_delta_LOR_min5.tsv")
