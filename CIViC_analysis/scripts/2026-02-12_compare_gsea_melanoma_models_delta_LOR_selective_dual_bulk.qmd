---
title: "GSEA comparison scClone2DR model vs. dual bulk"
...
-   **Option 1)** After the GSEA analysis, select (and plot) one clone per sample. This is only relevant for the scClone2DR publication model, because there can be several malignant clones. The dual bulk model has one malignant clone per sample.\
    With this, the same number of clones is plotted for scClone2DR publication model and dual bulk model, respectively.\
    Select clone with best GSEA score.

-   **Option 2)** Select only one drug and clone per sample. Select drug and clone that have highest GSEA score. This has a slightly different effect on dual bulk model and scClone2DR publication model. For the publication model only one clone per sample is selected (there can be several malignant clones). For the dual bulk model there is only one malignant clone per sample, and for each sample the best drug is selected.\
    The same number of clones is plotted for scClone2DR publication model and dual bulk model, respectively.\
...
-   **Option 3)** Select only one drug per sample, but keep all clones in the analysis, to show intra-sample heterogeneity. The drug is selected by the overall best GSEA score per sample. Similar to option 2), but showing all clones.\
    The number of data points for the dual bulk models is the same as number of samples, for the scClone2DR publication model a data point per malignant clone is shown. Therefore, the number of data points differs.

-   **Option 4)** Select the best drug per clone. For the dual bulk model this shows the same data as in Option 2) and 3). For the scClone2DR publication model, for each malignant clone the best drug is chosen independently.\
    The number of data points for the dual bulk models is the same as number of samples, for the scClone2DR publication model a data point per malignant clone is shown. Therefore, the number of data points differs.
...
# Sys.setenv(WORKING_DIRECTORY = "2023_cb_beerenwinkel_SDSC/scRNA/08_gsea/2026-02-12_gsea_delta_lor_comparison_selection_dual_bulk/")
...
dual_bulk_gsea_file <- paste0(input,
                         "dual_bulk_GSEA_delta_LOR_min5.tsv")

# read in tables
scClone2DR_gsea <- read.table(scClone2DR_gsea_file, header = T, sep = "\t" ) %>%
  mutate(model = if_else(model == "main", "scClone2DR", model))

dual_bulk_gsea <- read.table(dual_bulk_gsea_file, header = T, sep = "\t" )

gsea <- rbind(scClone2DR_gsea, dual_bulk_gsea) %>%
...
min_5 <- ggplot(gsea, aes(x = NES, colour = model, group = model)) +
  scale_colour_manual(values = c(dual_bulk = "#D55E00", scClone2DR = "#009E73")) +
  geom_density() +
  facet_wrap(~pathway) +
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_min_5_distribution_scClone2DR_vs_dual_bulk_jitter.pdf")
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_min_5_distribution_scClone2DR_vs_dual_bulk.pdf")
ggsave(min_5, filename = filename, width = 8, height = 5)
```
...
```{r}
# violin plot per drug showing both, scClone2DR and dual bulk model
...
dual_bulk_anchor <- gsea %>%
  filter(model == "dual_bulk")


seg_df <- gsea %>%
  filter(model == "scClone2DR") %>%
  left_join(dual_bulk_anchor, by = "sample", suffix = c(".scClone2DR", ".dual_bulk"),
            relationship = "many-to-many")
```

```{r}
# dot plot per drug showing both, scClone2DR and dual_bulk model
all_classes <- unique(table_drugs$drug_target_group)
pal_named <- setNames(brewer.pal(length(all_classes), "Paired")[seq_along(all_classes)], all_classes)
...
for (my_drug in unique(gsea$drug_delta_LOR)) {
  ...
  seg_df <- gsea %>%
    filter(drug_delta_LOR == my_drug)  %>%
    filter(model == "scClone2DR") %>%
    left_join(my_anchor, by = "sample", suffix = c(".scClone2DR", ".dual_bulk"))

  title_col <- pal_named[[my_class]]
  
  p_scatter <- ggplot(seg_df, aes(x = NES.dual_bulk, y = NES.scClone2DR)) +
  geom_point(
    alpha = 0.7,
    size = 1
  ) +
...
Test if the NES scores of the scClone2DR model are different from those of the dual_bulk model.

```{r}
gsea$model <- factor(gsea$model, levels = c("scClone2DR", "dual_bulk"))
```
...
wilcoxon_results <- gsea %>%
  ...
  group_modify(~{
    ...
    dir <- median(d$NES[d$model == "scClone2DR"], na.rm = TRUE) -
      median(d$NES[d$model == "dual_bulk"], na.rm = TRUE)
...
gsea_opt_1 <- rbind(scClone2DR_gsea_opt_1, dual_bulk_gsea) %>%
  # have sample column with sample ID
  mutate(sample = stringr::str_extract(clone, "^[^_]+"))
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_1_min_5_distribution_scClone2DR_vs_dual_bulk_jitter.pdf")
ggsave(min_5, filename = filename, width = 8, height = 5)
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_1_min_5_distribution_scClone2DR_vs_dual_bulk.pdf")
ggsave(min_5, filename = filename, width = 8, height = 5)
```
...
```{r}
gsea_opt_1$model <- factor(gsea_opt_1$model, levels = c("scClone2DR", "dual_bulk"))
```
...
Number of data points is the same for scClone2DR and dual_bulk model (one data point per sample).
...
# from dual_bulk model GSEA results: select drug with highest score
dual_bulk_gsea_opt_2 <- dual_bulk_gsea %>%
  # select best value per clone (sample)
  group_by(clone) %>%
  slice_max(order_by = NES, n = 1, with_ties = FALSE) %>%
  ungroup()

gsea_opt_2 <- rbind(scClone2DR_gsea_opt_2, dual_bulk_gsea_opt_2) %>%
  # have sample column with sample ID
  mutate(sample = str_extract(clone, "^[^_]+"))
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_2_min_5_distribution_scClone2DR_vs_dual_bulk_jitter.pdf")
ggsave(min_5, filename = filename, width = 8, height = 5)
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_2_min_5_distribution_scClone2DR_vs_dual_bulk.pdf")
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_2_min_5_violin_scClone2DR_vs_dual_bulk.pdf")
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_2_min_5_violin_scClone2DR_vs_dual_bulk_connecting_lines.pdf")
ggsave(my_p, filename = filename, width = 5, height = 3)
```
...
p_scatter <- ggplot(seg_df, aes(x = NES.dual_bulk, y = NES.scClone2DR)) +
  ...
  geom_point(
    ...
    aes(fill = drug_delta_LOR.scClone2DR,  # interior colour
      colour = drug_delta_LOR.dual_bulk          # outline colour
      )
  ) +
  labs(
    x = "NES (dual_bulk)",
  ...
  labs(fill   = "Best drug scClone2DR",
       colour = "Best drug dual_bulk")

print(p_scatter)

filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_2_min_5_scatter_scClone2DR_vs_dual_bulk.pdf")
...
gsea_opt_2$model <- factor(gsea_opt_2$model, levels = c("scClone2DR", "dual_bulk"))
```
...
dir <- median(gsea_opt_2$NES[gsea_opt_2$model == "scClone2DR"], na.rm = TRUE) -
      median(gsea_opt_2$NES[gsea_opt_2$model == "dual_bulk"], na.rm = TRUE)
...
# dual_bulk model: select best drug, based on best GSEA score per sample
dual_bulk_gsea_opt_3 <- dual_bulk_gsea %>%
  group_by(clone) %>%
  slice_max(order_by = NES, n = 1, with_ties = FALSE) %>%
  ungroup()

gsea_opt_3 <- rbind(scClone2DR_gsea_opt_3, dual_bulk_gsea_opt_3) %>%
  # have sample column with sample ID
  mutate(sample = str_extract(clone, "^[^_]+"))
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_3_min_5_distribution_scClone2DR_vs_dual_bulk_jitter.pdf")
ggsave(min_5, filename = filename, width = 8, height = 5)
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_3_min_5_distribution_scClone2DR_vs_dual_bulk.pdf")
ggsave(min_5, filename = filename, width = 6, height = 4)
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_3_min_5_violin_scClone2DR_vs_dual_bulk.pdf")
ggsave(my_p, filename = filename, width = 5, height = 3)
```
...
my_p <- ggplot(gsea_opt_3, aes(x = model, y = NES)) +
  ...
  geom_segment(
    data = seg_df,
    aes(x = "dual_bulk", xend = "scClone2DR", y = NES.dual_bulk, yend = NES.scClone2DR),
    inherit.aes = FALSE,
    linewidth = 0.2,
    alpha = 0.6
  ) +
  scale_fill_manual(values = c(dual_bulk = "#D55E00", scClone2DR = "#009E73")) +
  ggtitle("Best drug per sample (GSEA score), all clones, opt_3") +
  ylab("NES GSEA")
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_3_min_5_violin_scClone2DR_vs_dual_bulk_connecting_lines.pdf")
ggsave(my_p, filename = filename, width = 5, height = 3)
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_3_min_5_scatter_scClone2DR_vs_dual_bulk.pdf")
ggsave(p_scatter, filename = filename, width = 8, height = 5)
```

Scatter plot for publication (without best drug dual_bulk colouring).
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_3_min_5_scatter_scClone2DR_vs_dual_bulk_no_surrounds.pdf")
ggsave(p_scatter, filename = filename, width = 8, height = 5)
```
...
gsea_opt_3$model <- factor(gsea_opt_3$model, levels = c("scClone2DR", "dual_bulk"))
```
...
dir <- median(gsea_opt_3$NES[gsea_opt_3$model == "scClone2DR"], na.rm = TRUE) -
      median(gsea_opt_3$NES[gsea_opt_3$model == "dual_bulk"], na.rm = TRUE)
...
# dual_bulk model: select best drug, based on best GSEA score per sample
dual_bulk_gsea_opt_4 <- dual_bulk_gsea %>%
  group_by(clone) %>%
  slice_max(order_by = NES, n = 1, with_ties = FALSE) %>%
  ungroup()

gsea_opt_4 <- rbind(scClone2DR_gsea_opt_4, dual_bulk_gsea_opt_4) %>%
  mutate(sample = stringr::str_extract(clone, "^[^_]+"))
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_distribution_scClone2DR_vs_dual_bulk_jitter.pdf")
ggsave(min_5, filename = filename, width = 8, height = 5)
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_distribution_scClone2DR_vs_dual_bulk.pdf")
ggsave(min_5, filename = filename, width = 6, height = 4)
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_violin_scClone2DR_vs_dual_bulk.pdf")
ggsave(my_p, filename = filename, width = 5, height = 3)
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_violin_scClone2DR_vs_dual_bulk_connecting_lines.pdf")
ggsave(my_p, filename = filename, width = 5, height = 3)
```
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_scatter_scClone2DR_vs_dual_bulk.pdf")
ggsave(p_scatter, filename = filename, width = 8, height = 5)
```

Scatter plot for publication (without best drug dual_bulk).
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_scatter_scClone2DR_vs_dual_bulk_no_surrounds.pdf")
ggsave(p_scatter, filename = filename, width = 8, height = 5)
```
...
gsea_opt_4$model <- factor(gsea_opt_4$model, levels = c("scClone2DR", "dual_bulk"))
```
...
dir <- median(gsea_opt_4$NES[gsea_opt_4$model == "scClone2DR"], na.rm = TRUE) -
      median(gsea_opt_4$NES[gsea_opt_4$model == "dual_bulk"], na.rm = TRUE)
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_pie_scClone2DR_vs_dual_bulk.pdf")
...
filename <- paste0(output, my_indication, "_", "delta_LOR_gsea_opt_4_min_5_bar_scClone2DR_vs_dual_bulk.pdf")
