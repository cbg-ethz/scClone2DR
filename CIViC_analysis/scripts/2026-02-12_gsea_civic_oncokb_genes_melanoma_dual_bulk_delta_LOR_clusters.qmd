---
title: "GSEA of dual bulk model on civic gene sets"
...
# Sys.setenv(WORKING_DIRECTORY = "2023_cb_beerenwinkel_SDSC/scRNA/08_gsea/2026-02-12_gsea_delta_lor_dual_bulk/melanoma/")

working_directory <- Sys.getenv("WORKING_DIRECTORY")

input <- paste0(working_directory, "input/")
output <- paste0(working_directory, "output/dual_bulk/")
```
...
LOR_file <- paste0(input, "geneOncoKB_scatrex/bimodal__LOR.h5")
...
for (i_drug in seq_along(attrs$dim1_drugs)) {
  ...
  # Hydroxyurea = 19
  # i_drug = 19
  for (malclone in 1) {                       # In dual_bulk model there is only one malignant clone
    #malclone = 1
...
From sample: "MACEGEJ" "MACYHYS" "MADEGOD" .. And cluster/clone: 1 (dual_bulk has only one malignant clone). Now combined: "MACYHYS_1" "MADEGOD_1" ..
...
for (drug in seq_along(attrs$dim1_drugs)) {
  ...
  all_patients_list <- list()
    ...
    for (patient in seq_along(attrs$dim3_samples)) {
      ...
      # because dual_bulk is only one malignant clone,
      ...
      rownames(patient_data) <- paste0(attrs$dim3_samples[patient],
                                       "_",
                                       1) # dual_bulk has only one malignant clone
  ...
  # NOTE: For some drugs existing clones can have NaN for all features.
  ...
  if (any(filter_mask_rows)) print(paste0(attrs$dim1_drugs[[drug]], " has clones with only missing values."))
  if (any(filter_mask_rows)) print(rownames(md)[filter_mask_rows])
