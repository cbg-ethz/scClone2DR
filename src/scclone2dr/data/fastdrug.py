...
class FastDrug():
    def __init__(self, path_fastdrug, samples = None, concentration_drug = None):
        if path_fastdrug is None:
            ...
            if samples is not None:
                df = df.loc[df['SampleID'].apply(lambda x: x in samples)]
            if concentration_drug is not None:
                df = df.loc[df['Concentration']==concentration_drug]
            self.sample_names = np.unique(df['SampleID'].values)
            ...
            self.selected_drugs = [str(d) for d in drug_mapping.values() if (d!='DMSO' and not('|' in d))]
    ...
    def load_FD_data(self, sample_names, concentration_DMSO='100', concentration_drug='10'):
        ...
        print('Number of drugs', len(self.selected_drugs))
        ...
        self.sample2nb_cells_per_well, self.sample2nb_cells_per_well_control, self.sample2tumor, self.sample2tumor_control, self.sample2wellpos, self.sample2control_wellpos = sample2nb_cells_per_well, sample2nb_cells_per_well_control, sample2tumor, sample2tumor_control, sample2wellpos, sample2control_wellpos
