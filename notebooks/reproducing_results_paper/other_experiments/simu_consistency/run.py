...
ls_R = [i for i in range(5,101,5)]
#ls_neg_bin = list(range(1,11))
ls_neg_bin = [0.5, 0.1, 0.01, 0.001]


for ite in range(40):
    for neg_bin_n in ls_neg_bin:
        np.random.seed(1)
        data_ref = get_simulated_training_data({'C':24,'R':1,'N':100,'Kmax':7, 'D':30, 'theta_rna':15}, neg_bin_n=neg_bin_n)
        ...
        for R in ls_R:
            ...
            import os
            directory = '/data/users/quentin/Paper_WP1/expes_paper/simu_consistency/save/R_'+str(R)+'negb_'+str(neg_bin_n)+'/'
            try:
            ...
            try:
                DIC, DIC_sample = over_sample(data_ref, multi_C = 1, multi_R = R, mode_nu=mode_nu, mode_theta=mode_theta)
