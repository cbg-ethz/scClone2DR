...
gene_set_collections = ['c6','c2_pid','hallmarks','gene'] #['c6','hallmarks', 'c2_pid', 'c2_kegg_medicus']
cohort2clonemodes = {'melanoma': ['scatrex'], 'aml':['phenograph']}

penalties_l1 = [0.2*2**i for i in range(0,10)]
penalties_l2 = [0.05*2**i for i in range(0,10)]



stabilityname = 'hyperparam_optim'
                    ...
                    for ite in range(1,20):
                        try:
                            ...
                            if gene_set_collection=='gene':
                                params_svi = model.train(data_train, penalty_l1=penalty_l1, penalty_l2=penalty_l2, n_steps=1000)
                            else:
                                params_svi = model.train(data_train, penalty_l1=penalty_l1, penalty_l2=penalty_l2 , n_steps=1000)
