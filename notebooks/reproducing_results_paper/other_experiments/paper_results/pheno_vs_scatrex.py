        ...
        for gene_set_collection in gene_set_collections:
            ...
            for pl1, penalty_l1 in tqdm(enumerate(penalties_l1)):
                for penalty_l2 in penalties_l2:
                    ...
                    tempdir = os.path.join(rootpath,'{0}/{1}_{2}_sparser/{3}/'.format(COHORT, gene_set_collection, clonemode, stabilityname))
                    ...
                    for ite in range(0,100):
                        try:
                            if clonemode=='scatrex' and gene_set_collection=='gene':
                                ...
                                with open(os.path.join(rootpath,'{0}/{1}_{2}_sparser/stability_paper/params_svi_ite_{3}_l1_12.8_l2_3.2.pkl'.format(COHORT, 'gene', 'scatrex', ite)), 'rb') as handle:
                            ...
                            with open(os.path.join(rootpath,'{0}/{1}_{2}_sparser/{6}/params_svi_ite_{5}_l1_{3}_l2_{4}.pkl'.format(COHORT, gene_set_collection, clonemode, str(penalty_l1), str(penalty_l2), ite, stabilityname)), 'wb') as handle:
